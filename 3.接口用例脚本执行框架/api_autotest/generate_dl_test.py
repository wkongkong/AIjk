...
用途：为 dl_ 开头的测试用例文件自动生成对应的独立测试脚本

使用方式：
    python generate_dl_test.py data_yaml/dl_test.yaml
    python generate_dl_test.py data_csv/dl_login.csv
"""
...
def generate_test_script(data_file_path):
    """
    生成独立测试脚本
    
    Args:
        data_file_path: 测试用例文件路径（相对于项目根目录）
    """
    # 检查文件是否存在
    if not os.path.exists(data_file_path):
        print(f"❌ 错误: 文件不存在 - {data_file_path}")
        return False
    ...
    # 检查是否以 dl_ 开头
    if not file_name_without_ext.startswith('dl_'):
        print(f"⚠️  警告: 文件名不以 dl_ 开头 - {file_name}")
        print("   独立测试脚本仅用于 dl_ 开头的测试用例文件")
        return False
    
    # 生成测试脚本文件名
    test_script_name = f"{file_name_without_ext}.py"
    test_script_path = os.path.join("testcases", test_script_name)
    
    # 检查测试脚本是否已存在
    if os.path.exists(test_script_path):
        response = input(f"⚠️  测试脚本已存在: {test_script_path}\n   是否覆盖? (y/n): ")
        if response.lower() != 'y':
            print("❌ 取消生成")
            return False
    ...
    script_content = f'''"""
独立测试脚本 - {file_name}
对应测试用例文件：{data_file_path}

使用方式：
    pytest testcases/{test_script_name} -v
"""
...
# 加载测试用例数据
TEST_DATA_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), "{data_file_path.replace(os.sep, '/')}")
test_cases = DataHandler.load_test_cases(TEST_DATA_FILE)


@allure.feature("API测试")
@allure.story("{file_name_without_ext}接口测试")
class Test{file_name_without_ext.replace('_', '').title()}:
    """{file_name} 独立测试类"""
    ...
    def test_api(self, test_case, api_client):
        ...
        Args:
            test_case: 测试用例数据（从{file_name}加载）
...
'''
    ...
    print(f"✅ 成功生成测试脚本: {test_script_path}")
    print(f"\n使用方式:")
    print(f"  pytest {test_script_path} -v")
    print(f"  pytest {test_script_path} --alluredir=allure-results -v")
...
def main():
    """主函数"""
    if len(sys.argv) < 2:
        print("用法: python generate_dl_test.py <测试用例文件路径>")
        print("\n示例:")
        print("  python generate_dl_test.py data_yaml/dl_test.yaml")
        print("  python generate_dl_test.py data_csv/dl_login.csv")
        sys.exit(1)
    ...
    print("自动生成独立测试脚本工具")
    print("=" * 60)
    print(f"\n测试用例文件: {data_file_path}")
    print()
    
    success = generate_test_script(data_file_path)
